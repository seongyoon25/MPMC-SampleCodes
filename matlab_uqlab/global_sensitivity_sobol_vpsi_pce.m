%{
    File Name: global_sensitivity_sobol_vpsi_pce.m
    ...
    Vector projection-based sensitivity index (VPSI) is used for 
...
%% Calculate Ynorm (= Y bar) and Correlation Matrix
yNorm = metaOpts.ExpDesign.Y ./ std(metaOpts.ExpDesign.Y, 1);
corrMat = cov(yNorm);
metaOpts.ExpDesign.Y = yNorm;

%% Start UQ Lab PCE and save
myPCE = uq_createModel(metaOpts);
save('result_GSA_PCE_norm.mat', 'myPCE');
...
%% Get vector projection-based sensitivity indices (VPSIs)
totalVPSI = [];
for parIdx = 1:length(parNames)
    vTotalAllIndivPar = (pceSobolAnalysis.Results.Total(parIdx, :) .* pceSobolAnalysis.Results.TotalVariance)';
    vTotalAll = pceSobolAnalysis.Results.TotalVariance';
    parVPSI = (vTotalAllIndivPar' * corrMat * vTotalAll) ./ (vTotalAll' * corrMat * vTotalAll);
    totalVPSI = [totalVPSI, parVPSI];
end

%% Plot the results
figure
bar(totalVPSI)
title('Vector Projection-based Sensitivity Index')
